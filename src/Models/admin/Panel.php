...
class Panel {
    public function index($db) {
        $userData = $db->query("SELECT `users`.`id`, `users`.`name`, `users`.`email`, `user_type`.`type` FROM `users` INNER JOIN `user_type` ON `users`.`type_id` = `user_type`.`id` WHERE `user_type`.`id` >= 1");
