...
<body class="min-h-screen">
    ...
    <div class="container mx-auto py-5 px-10 bg-slate-50 flex justify-center signup-container">
        <div class="signup">
            <form method="post" action="/sign-up" class="flex flex-col">
                <?php
                    if (isset($_POST['name_error'])) {
                        echo '<div class="text-red-500">' . $_POST['name_error'] . '</div>';
                    } 
                    if (isset($_POST['email_error'])) {
                        echo '<div class="text-red-500">' . $_POST['email_error'] . '</div>';
                    }
                ?>
                <div class="flex block flex-col w-80 mb-5">
                    <label for="name" class="mb-3">Name</label>
                    <input required class="py-2 px-3" type="text" name="name" placeholder="name" value="<?php echo isset($_POST['name']) ? $_POST['name'] : '';?>">
                </div>
                <div class="flex block flex-col w-80 mb-5">
                ...
                <button type="submit" class="text-slate-100 hover:bg-sky-700 hover:shadow-md cursor-pointer bg-sky-500 px-3 py-1 rounded w-40 m-auto">sign up</button>
            </form>
        </div>
    </div>
    </div>
